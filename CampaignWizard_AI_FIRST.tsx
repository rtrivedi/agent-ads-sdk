...
const STEPS = [
  ...
  "Context Triggers",
  "Target Audience",
  "Provide Your Offer", // Changed from "Ad Creative"
  "Review & Launch"
];
...
const CampaignWizard: React.FC = () => {
  ...
  const form = useForm<CampaignFormData>({
    ...
    defaultValues: {
      ...
      adType: "link",
      landingUrl: "", // Where to send users
      promoCode: "", // Optional promo/tracking code
      keyBenefits: [], // Array of key benefits (NEW)
      specialOffer: "", // Free trial, discount, etc (NEW)
      successMetrics: "", // Proof points, testimonials (NEW)
      ...
      // Legacy fields (we'll map these from the new approach)
      adTitle: "",
      adDescription: "",
      cta: "Learn More",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // For AI-first approach, we'll construct a dynamic title and description from the provided info
      // The AI will use these as fallbacks but can craft its own messages
      const autoTitle = watched.valueProposition || `${watched.campaignName} - Special Offer`;
      const autoDescription = watched.keyBenefits?.join('. ') || watched.intentDescription;
      ...
      const payload: Record<string, any> = {
        ...
        // NEW: AI-first offer information (stored in metadata or custom fields)
      ...
      if (adType === "service") {
        ...
        // For link ads, use auto-generated or fallback content
        payload.title = autoTitle.substring(0, 60); // Limit to 60 chars
        payload.body = autoDescription.substring(0, 150); // Limit to 150 chars
        payload.cta = "Learn More"; // AI will choose appropriate CTA
        payload.landing_url = watched.landingUrl;
        payload.promo_code = watched.promoCode || null;
      }
          ...
          <>
            ...
            <div>
              ...
              <TagInput
                ...
                onChange={(tags) => form.setValue("keyBenefits", tags)}
  ...
  const stepComponents = [
    ...
    <Step5Offer key="offer" />, // Changed from Step5Creative
