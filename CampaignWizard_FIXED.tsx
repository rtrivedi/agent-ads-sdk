...
import { Loader2, ArrowRight, ArrowLeft, Info, X } from "lucide-react";
...
const CampaignWizard: React.FC = () => {
  ...
  // Note: You may need to extend your CampaignFormData type to include all these fields
  const form = useForm<CampaignFormData>({
    ...
    defaultValues: {
      ...
      budget: "10", // Default $10
      bidAmount: "0.50", // Default $0.50
  ...
  // Determine effective ad type based on promo code
  ...
  const validateStep = (currentStep: number): boolean => {
    switch(currentStep) {
      ...
      case 3: // Context Triggers - At least 1 trigger required!
        return !!(
      ...
      case 5: // Creative
        ...
        return !!(
          watched.adTitle &&
          watched.adTitle.length >= 5 &&
          watched.adTitle.length <= 60 &&
          watched.adDescription &&
          watched.adDescription.length >= 20 &&
          watched.adDescription.length <= 150 &&
          watched.cta &&
          watched.cta.length >= 2 &&
          watched.landingUrl &&
    ...
    switch(currentStep) {
      ...
      case 5:
        if (adType === "service") {
          ...
          if (!watched.adTitle || watched.adTitle.length < 5)
            return "Headline must be at least 5 characters";
          if (watched.adTitle && watched.adTitle.length > 60)
            return "Headline must be 60 characters or less";
          if (!watched.adDescription || watched.adDescription.length < 20)
            return "Description must be at least 20 characters";
          if (watched.adDescription && watched.adDescription.length > 150)
            return "Description must be 150 characters or less";
          if (!watched.landingUrl || !watched.landingUrl.startsWith("https://"))
            return "Landing URL must start with https://";
        }
        return "";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload: Record<string, any> = {
        ...
        budget: budgetCents, // Convert to cents
        bid_cpc: bidCents, // Convert to cents
        ad_type: effectiveAdType,

        // Context fields for embeddings (these field names must match backend)
        intent_description: watched.intentDescription,
        ideal_customer: watched.idealCustomer,
        problem_solved: watched.problemSolved, // Make sure this field name matches backend
        value_proposition: watched.valueProposition,

        // Arrays - ensure they're always arrays even if empty
      ...
      if (adType === "service") {
        ...
        payload.service_endpoint = watched.serviceEndpoint || null;
        payload.service_auth = watched.serviceAuth || null;
      } else {
        payload.title = watched.adTitle;
        payload.body = watched.adDescription;
        payload.cta = watched.cta;
        payload.landing_url = watched.landingUrl;
        payload.promo_code = effectiveAdType === "recommendation" ? watched.promoCode : null;
      }

      console.log("Submitting campaign payload:", payload); // Debug log
      ...
      <div className="space-y-4">
        <div>
          ...
          {form.formState.errors.campaignName && (
            <p className="text-red-500 text-xs mt-1">{form.formState.errors.campaignName.message}</p>
          )}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">
            Add contexts that indicate when someone needs your solution
          </p>
        </div>
        ...
        {adType === "service" ? (
          ...
          <>
            ...
            <div>
              ...
              />
              {watched.landingUrl && !watched.landingUrl.startsWith("https://") && (
            ...
            <div>
              ...
              />
              {watched.promoCode && watched.promoCode.length >= 3 && (
                <p className="text-green-600 text-xs mt-1">
                  ✓ This will become a recommendation ad with promo code
                </p>
              )}
            </div>
          </>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium capitalize">{effectiveAdType}</span>
            </div>
          </div>
        </div>
